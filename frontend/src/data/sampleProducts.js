  ...
  categories.forEach(category => {
    ...
    for (let i = 0; i < productsPerCategory && productIndex < count; i++) {
      ...
      const product = {
        ...
        price: price,
        image: getProductImage(productName), // Use smart image mapping
        category: category,
...
export function initializeDemoProducts() {
  const PRODUCT_VERSION = '3.0'; // Increment this to force regeneration
  const storedVersion = localStorage.getItem('w3mart_product_version');
  
  // Force regeneration if version changed
  if (storedVersion !== PRODUCT_VERSION) {
    console.log('🔄 Product version changed, regenerating all products...');
    localStorage.removeItem('w3mart_seller_products');
    localStorage.setItem('w3mart_product_version', PRODUCT_VERSION);
  }
  
  const existingProducts = JSON.parse(localStorage.getItem('w3mart_seller_products') || '[]');
  
  // CRITICAL FIX: ALWAYS fix product images on every load
  console.log('🖼️  Fixing product images to match names...');
  let imagesFixed = 0;
  const productsWithFixedImages = existingProducts.map(p => {
    const correctImage = getProductImage(p.name);
    if (p.image !== correctImage) {
      imagesFixed++;
      console.log(`   ✓ Fixed: ${p.name}`);
      return { ...p, image: correctImage };
    }
    return p;
  });
  
  if (imagesFixed > 0) {
    console.log(`✅ Fixed ${imagesFixed} product images`);
    localStorage.setItem('w3mart_seller_products', JSON.stringify(productsWithFixedImages));
  } else {
    console.log('✅ All product images already correct');
  }
  
  // STEP 2: Fix old products with wrong seller email
  let needsUpdate = false;
  const fixedProducts = productsWithFixedImages.map(p => {
  ...
  // STEP 3: Check if demo products already exist
  ...
  console.log(`✅ Found ${fixedProducts.length} existing products with correct images`);
