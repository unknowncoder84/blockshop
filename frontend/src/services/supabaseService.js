...
class SupabaseService {
  ...
  /**
   * Add product review
   * Enhanced: Only allows reviews for delivered orders
   */
  async addReview(reviewData) {
    console.log('⭐ ADD REVIEW ATTEMPT:', reviewData);
    
    if (!this.isConnected || this.isDemoMode) {
      // Check if user has received this product (order must be delivered)
      const storedOrders = JSON.parse(localStorage.getItem('w3mart_orders') || '[]');
      const deliveredOrder = storedOrders.find(order => 
        order.product_id === reviewData.product_id &&
        order.buyer_email === reviewData.user_email &&
        order.status === 'delivered'
      );
      
      if (!deliveredOrder) {
        console.log('❌ Cannot review - no delivered order found');
        return {
          success: false,
          error: 'You can only review products you have received. Please wait for your order to be delivered.'
        };
      }
      
      // Store review in localStorage
      const storedReviews = JSON.parse(localStorage.getItem('w3mart_reviews') || '[]');
      const newReview = {
        id: 'review-' + Date.now(),
        ...reviewData,
        created_at: new Date().toISOString()
      ...
      localStorage.setItem('w3mart_reviews', JSON.stringify(storedReviews));
  ...
  /**
   * Authenticate user
   * Enhanced: Validates against stored users in localStorage
   */
  ...
  /**
   * Register user
   * Enhanced: Stores user in localStorage and validates uniqueness
   */
  ...
  getMockProducts() {
    ...
    if (storedProducts.length > 0) {
      // Fix images for all products
      const productsWithCorrectImages = storedProducts.map(p => ({
        ...p,
        image: getProductImage(p.name)
      }));
      
      console.log(`✅ Loaded ${productsWithCorrectImages.length} products with correct images`);
      return productsWithCorrectImages;
    }
