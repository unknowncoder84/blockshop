...
export default function ReviewSection({ productId, user, canReview = false }) {
  ...
  const checkIfUserCanReview = useCallback(async () => {
    ...
    try {
      ...
      const orders = await supabaseService.getUserOrders(user.email || user.id);
      const deliveredOrder = orders.find(order => 
        order.product_id === productId &&
        order.status === 'delivered'
      );
      
      setHasDeliveredOrder(!!deliveredOrder);
      console.log('🔍 Can user review?', !!deliveredOrder, 'Product:', productId);
    } catch (error) {
  ...
  }, [user, canReview, productId]);

  useEffect(() => {
    loadReviews();
    checkIfUserCanReview();
  }, [loadReviews, checkIfUserCanReview]);
  ...
  const handleSubmitReview = async (e) => {
    ...
    if (!hasDeliveredOrder) {
      toast.error('Cannot submit review', {
        description: 'You can only review products you have received. Please wait for your order to be delivered.',
        duration: 5000
      });
      return;
    }
            ...
            {checkingOrder ? (
              ...
              <div className="text-right">
                <Button
                  ...
                  title="You can only review products you have received"
                >
                  Write a Review
                </Button>
